...
impl ExchangeService {
    ...
    pub async fn import_ccv2_json<C>(db: &C, bytes: &[u8]) -> Result<ImportedCharacter, CoreError>
    where
        C: ConnectionTrait + TransactionTrait,
    {
        ...
        let imported = import_into(&transaction, character, book).await?;
        transaction.commit().await?;
        Ok(imported)
    }

    /// 导入 PNG 角色卡:读 `chara` chunk 后与 JSON 同路。
    pub async fn import_ccv2_png<C>(db: &C, png: &[u8]) -> Result<ImportedCharacter, CoreError>
    where
        C: ConnectionTrait + TransactionTrait,
    {
        let json = crate::exchange::png::read_card(png)?;
        Self::import_ccv2_json(db, &json).await
    }
    ...
    pub async fn export_ccv2_png(
        ...
        let json = Self::export_ccv2_json(db, character_id).await?;
    ...
    pub async fn export_ccv2_json(
        ...
        let loaded = AssetService::load::<CharacterDefinition>(db, character_id).await?;
        let CharacterDefinition::V1(character) = loaded.definition;
        let book = match character.world_books.first() {
            Some(reference) => {
                let loaded = AssetService::load::<WorldBookDefinition>(db, reference.id()).await?;
...
pub(crate) async fn import_into(
    ...
    let world_book = match book {
        Some(book) => {
            ...
            let head =
                AssetService::create(db, &name, None, &WorldBookDefinition::V1(book)).await?;
    ...
    let head = AssetService::create(db, &name, None, &CharacterDefinition::V1(character)).await?;
